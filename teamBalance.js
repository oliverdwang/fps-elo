...
function selectTeams() {
  ...
  // Origin sheet
  var teamSelectionSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Team Selection");
  var teamOneNamesOriginRange = teamOneSelOutputRange.strCol + // First cell
                          Number(teamOneSelOutputRange.strRow +
                          teamSelectionPlayersRowOffset + teamSelectionOptionRowSpacing*(Number(id)-1)) +
                          ":" + teamOneSelOutputRange.strCol + // Last cell
                          Number(teamOneSelOutputRange.strRow +
                          teamSelectionPlayersRowOffset + teamSelectionOptionRowSpacing*(Number(id)-1) + 4);
  var teamTwoNamesOriginRange = teamTwoSelOutputRange.strCol + // First cell
                          Number(teamTwoSelOutputRange.strRow +
                          teamSelectionPlayersRowOffset + teamSelectionOptionRowSpacing*(Number(id)-1)) +
                          ":" + teamTwoSelOutputRange.strCol + // Last cell
                          Number(teamTwoSelOutputRange.strRow +
                          teamSelectionPlayersRowOffset + teamSelectionOptionRowSpacing*(Number(id)-1) + 4);
  var teamOneNames = teamSelectionSheet.getRange(teamOneNamesOriginRange).getValues();
  var teamTwoNames = teamSelectionSheet.getRange(teamTwoNamesOriginRange).getValues();
  addAMatchSheet.getRange(teamOneNamesRange.toString()).setValues(teamOneNames);
  addAMatchSheet.getRange(teamTwoNamesRange.toString()).setValues(teamTwoNames);
